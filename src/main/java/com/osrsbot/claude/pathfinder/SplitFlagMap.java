...
/**
 * Splits a large flag map into regions that are lazily decompressed and cached.
 * Uses a Guava LoadingCache with a 20MB weight limit to keep memory bounded.
 *
    ...
    public SplitFlagMap(int regionSize, Map<Position, byte[]> compressedRegions, int flagCount)
    {
        ...
        regionMaps = CacheBuilder
            ...
            .build(CacheLoader.from(position -> {
                ...
                {
                    return new FlagMap(
                        position.x * regionSize, position.y * regionSize,
                        (position.x + 1) * regionSize - 1, (position.y + 1) * regionSize - 1,
                        this.flagCount);
                }
